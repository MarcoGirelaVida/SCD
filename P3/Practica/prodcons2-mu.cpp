...
const int
   ...
   num_items             = 20,   // Es multiplo del numero de productores y consumidores
   valores_por_productor = num_items / NUM_PRODUCTORES , 
   valores_por_consumidor= num_items / NUM_CONSUMIDORES ,
   tam_vector            = 10;
...
int producir(int id_productor)
{
   ...
   contador++ ;
   cout << "Productor: " << (id_productor / 2) << " ha producido valor " << contador << endl << flush;
   return contador ;
}
// ---------------------------------------------------------------------
...
void funcion_productor(int id_productor)
{
   for ( unsigned int i = 0 ; i < valores_por_productor ; i++ )
   {
      ...
      // enviar valor
      cout << "Productor: " << (id_productor / 2) << " va a enviar valor " << valor_prod << endl << flush;
      MPI_Ssend( &valor_prod, 1, MPI_INT, id_buffer, etiq_productor, MPI_COMM_WORLD );
   }
   cout << "\t\t\tPRODUCTOR: " << (id_productor / 2) << " FINALIZÓ" << endl;
}
// ---------------------------------------------------------------------
...
void funcion_consumidor(int id_consumidor)
{
   ...
   for( unsigned int i = 0 ; i < valores_por_consumidor; i++ )
   {
      ...
      cout << "Consumidor: " << (id_consumidor / 2) << " ha recibido valor " << valor_rec << endl << flush ;
      consumir( valor_rec );
   }

   cout << "\t\t\tCONSUMIDOR: " << (id_consumidor / 2) << " FINALIZÓ" << endl;
}
// ---------------------------------------------------------------------
...
int main( int argc, char *argv[] )
{
   ...
   if ( num_procesos_esperado == num_procesos_actual )
   {
      ...
      else                                // Si es par y no es el 8, se productor
         funcion_productor(id_propio);    //  && (id_propio != 8) Lo de distinto de 8 es para que sean 4 en vez de 5
   }
   else
   {
