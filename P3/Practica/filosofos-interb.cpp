...
void funcion_filosofos( int id )
{
   int id_ten_izq = (id+1)  % num_filo_ten, //id. tenedor izq.
      id_ten_der = (id-1)  % num_filo_ten; //id. tenedor der.
   int valor = 0;
   ...
   while ( true )
   {
      ...
      MPI_Ssend(&valor, 1, MPI_INT, id_ten_izq, MPI_ANY_TAG, MPI_COMM_WORLD);

      cout <<"Filósofo " <<id <<" solicita ten. der." <<id_ten_der <<endl;
      // ... solicitar tenedor derecho (completar)
      MPI_Ssend(&valor, 1, MPI_INT, id_ten_der, MPI_ANY_TAG, MPI_COMM_WORLD);
...
int main( int argc, char** argv )
{
   ...
   if ( num_procesos == num_procesos_actual )
   {
      ...
      if ( id_propio % 2 == 0 )          // si es par (por tanto cualquiera de una mitad de los procesos)
